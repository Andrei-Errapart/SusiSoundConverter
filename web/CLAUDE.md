...
Vue 3 + TypeScript single-page app for viewing and editing IntelliSound sound files (.DS3, .DX4, .DSU, .DS6) and DHE sound files (.DHE). Deployed to GitHub Pages via CI.
...
src/
  lib/           Pure functions, no framework dependencies
    parser.ts      Binary → SoundFile (all formats including DHE)
    serializer.ts  SoundFile → binary (all formats including DHE)
    xor.ts         XOR scrambling/descrambling
    constants.ts   Header offsets, sample rates, flash sizes
    audio.ts       Web Audio playback (8-bit and 16-bit PCM)
    wav-import.ts  WAV/MP3 → target format PCM + cross-format conversion
    crc32.ts       CRC32 for track fingerprinting
    track-utils.ts Duration formatting (format-aware)
  types/
    sound-file.ts  SoundFile, Track, TrackTable, DheRecord, TrackTableKind
  composables/   Vue composables (stateful)
    useSoundFile.ts  File load/export, flash usage tracking
    useCopyPaste.ts  Cross-pane track selection (carries source format info)
    useAudioPlayer.ts Track playback state
  components/    Vue SFCs
    FilePane.vue     Top-level per-file container (load, export, overwrite, import)
    TrackTable.vue   Renders one track table (primary, extended, middle, dhe_tracks, etc.)
...
- **SoundFile**: parsed representation of a .DS3/.DX4/.DSU/.DS6/.DHE binary file
- **Track**: one audio clip — `{ audio: Uint8Array, loopOffset: number, ... }`
- **TrackTable**: a group of track slots (primary, extended, middle, dsu, ds6_ext1/2/3, dhe_tracks)
...
- **Two audio formats**:
  - IntelliSound (DS3/DX4/DSU/DS6): 8-bit unsigned mono PCM at 13,021 Hz; silence = 0x80
  - DHE (X-clusive PROFI): 16-bit signed LE mono PCM at 22,050 Hz; 128 Mbit flash
- `SoundFile.sampleRate` and `SoundFile.bitDepth` determine the format
- Cross-format track copying automatically resamples via `convertTrackAudio()`
- WAV/MP3 import resamples to the target file's format
- All header data is XOR-scrambled: `byte XOR (offset & 0xFF)`
- DHE 11-byte track records at 0x800 (vs 3-byte entries in IntelliSound)
...
- Test data lives in `../../test_data/` (IntelliSound) and `../../test_data/DEH/` (DHE files)
